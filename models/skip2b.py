...
#         print(model_tmp)
        ...
        if num_channels_skip[i] != 0:
            ...
            skip.add(nn.Upsample(scale_factor=2**(i+1), mode=upsample_mode[i]))
...
#         print(model_tmp)
        k = (num_channels_up[i - 1] + num_channels_skip[i - 1] if i > 0 else input_depth)
        if num_channels_up[i] != 0:  # for debugging only
            inner.add(bn(k))
    ...
    model.add(model_tmp)
    model.add(conv(num_channels_up[-1] + num_channels_skip[-1], num_output_channels, 1, bias=need_bias, pad=pad))
    if need_sigmoid:
